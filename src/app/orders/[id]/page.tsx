...
export default function OrderDetailPage({ params }: Props) {
  ...
  if (!order) {
    return (
      <div className="mx-auto w-full max-w-xl p-4">
        <p>Order not found.</p>
        <Link href="/orders" className="mt-3 inline-block underline">Back to orders</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-xl">
      ...
      <div className={`${ui.header} sticky top-0 z-10 px-4 py-3 text-white`}>
        <div className="mx-auto flex max-w-xl items-center justify-between">
          ...
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-white/20">
            <svg viewBox="0 0 24 24" width="16" height="16" className="text-white/90">
        ...
        <div className="mx-auto mt-3 flex max-w-xl items-center justify-center gap-6 pb-1">
          ...
          <Step icon="pending" label="Pending" active={order.status !== OrderStatus.Canceled} current={order.status === OrderStatus.Pending} />
          <StepConnector active={order.status !== OrderStatus.Pending} />
          {order.status === OrderStatus.Canceled ? (
            <Step icon="canceled" label="Canceled" active />
          ) : (
            <Step icon="completed" label="Completed" active={order.status === OrderStatus.Completed} />
          )}
        </div>
      </div>
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate text-[15px] font-semibold">Apple Gift Card</div>
            <div className="text-sm text-muted-foreground">USA • $100</div>
          </div>
        </div>
      ...
      {/* Sticky actions */}
      <div className="fixed inset-x-0 bottom-0 z-20 border-t bg-background/95 px-4 pb-6 pt-3 backdrop-blur">
        <div className="mx-auto w-full max-w-xl space-y-3">
          ...
          <button className="w-full rounded-2xl border px-4 py-3 text-base font-semibold hover:bg-accent/40">
            Some other action
          </button>
        </div>
      </div>
...
function Step({
  ...
  const glyph =
    icon === "paid" ? (
      <svg width="16" height="16" viewBox="0 0 24 24"><path d="M4 7h16M4 12h10M4 17h7" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
    ) : icon === "pending" ? (
      <svg width="16" height="16" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="2" fill="none"/><path d="M12 7v5l3 3" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
    ) : icon === "completed" ? (
      <svg width="16" height="16" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
    ) : (
      <svg width="16" height="16" viewBox="0 0 24 24"><path d="M6 6l12 12M18 6L6 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
    );
  return (
  ...
  return (
    <button
      className={`inline-flex items-center gap-1 rounded-xl px-3 py-1.5 text-xs ${
        disabled ? "cursor-not-allowed bg-muted text-muted-foreground" : "bg-muted hover:bg-muted/70"
      }`}
      disabled={disabled}
    >
