	...
	return (
		<div className="fixed inset-x-0 bottom-0 z-50 border-t bg-white/90 backdrop-blur">
			<div className="/* respect iOS home indicator */ mx-auto flex max-w-xl items-center justify-between gap-6 px-4 pt-3 pb-[max(14px,env(safe-area-inset-bottom))]">
				{/* Left circular */}
				<SideBtn to={homeHref} onClick={onHomeClick}>
					{/* Home icon */}
				...
				<Link
					...
					className="flex items-center gap-2 rounded-full bg-[#CC4B00] px-6 py-3 font-semibold text-white shadow-[0_10px_28px_rgba(204,75,0,0.35)] transition hover:opacity-95"
				>
					{/* Box icon */}
				...
				{/* Right circular */}
				<SideBtn to={settingsHref} onClick={onSettingsClick}>
					{/* Settings icon */}
					<svg width="22" height="22" viewBox="0 0 24 24">
						<path
